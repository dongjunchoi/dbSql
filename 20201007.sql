...
SELECT문 처리 순서
1. 실행계획 생성 OR 기 생성된 찾기
2. 바인드 변수처리
3. 실행
4. **********  인출

커서를 통해 개발자가 인출 하는 과정을 통제함으로 써 테이블 타입변수에 
SELECT 결과를 모두 담지 않고도(메모리 낭비없이) 효육적으로 처리하는 것이 가능하다.

커서의 종류
1. 묵시적 커서 - 커서 선업없이 실행한 SQL에 대해 오라클 서버가 스스로 생성, 삭제하는 커서
2. 명시적 커서 - 개발자가 선언해서 사용하는 커서
...
--DEPT 테이블의 모든 부서번호, 부서이름을 CURSOR를 통해 데이터 출력
DECLARE
    /*커서 선언*/
    CURSOR c_dept IS 
        SELECT deptno, dname
...
명시적 커서 FOR LOOP : FOR LOOP와 명시적 커서를 결합한 형태로 커서 OPEN,FETCH,CLOSE 단계를 FOR LOOP에서 처리
                    개발자가 사용하기 쉬운 형태로 제공
사용방법(JAVA 향상된 FOR문과 비슷)
...
쿼리가 짧을 경우 FOR LOOP에 커서를 인라인 형태로 작성하여 사용가능 ==> DECLARE 절에 커서를 선언하지 않음
...
avgdt와 동일한 기능을 하는 SELECT쿼리를 분석함수를 이용하여 작성
SELECT dt, LEAD(dt) OVER (ORDER BY dt DESC) dt_lead,
        dt- LEAD(dt) OVER (ORDER BY dt DESC) diff_avg
FROM dt;
----dt- LEAD(dt) OVER (ORDER BY dt DESC) diff_avg 평균구하기--
SELECT AVG(diff)
FROM (SELECT dt- LEAD(dt) OVER (ORDER BY dt DESC) diff
FROM dt);

분석함수 없이작성
select max(dt),min(dt),count(dt)
from dt;
