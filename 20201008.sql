Exception (예외) : pl/sql 블럭이 실행되는 동안 발생하는 에러
                 예외가 발생 했을 때 PL/SQL의 EXCEPTION 절을 기술하여 다른 로직을 실행하는 예외 문제를 해결할 수 있다.

예외 구분
1. 사전 정의 예외(Oracle Predefined Exception)
        오라클에서 미리 정의한 예외로 ORA-XXXXX로 정의, 에러코드만 있고 에러이름이 없는 경우가 대다수
...
--하나의 행이 조회되어야 하는 상황에서 여러개의 행이 반환된 경우(예외처리가 없을때)
SET SERVEROUTPUT ON;
...
EXCEPTION
    ...
    WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('NO_MANY_ROWS');
END;
/

사용자 예외 정의, 사용
1. PL/SQL 블럭안에서 또다른 PL/SQL 블럭을 정의 하는 것이 가능하다.
--NO_EMP : 사번을 통해서 사원을 검색하는데 사번을 갖는 사원이 없을때
    SELECT *
    FROM emp
    WHERE empno = -1;
    ==>no_data_found ==>no_demp
DECLARE
    NO_EMP EXCEPTION;
    v_ename emp.ename%TYPE;
BEGIN
...
사원이 속한 부서이름 가져오는 방법(다른 테이블의 컬럼을 확장 하는 방법)
1. join
...
데이터 분포도 : 특정 데이터가 발생하는 빈도
    ex) emp테이블의 empno : 값 - 14개 존재  - 데이터 분포도가 좋다.
        emp테이블의 deptno : 값 - 3개 존재  - 데이터 분포도가 나쁘다.(중복된 값이 많다)
        
함수의 경우 오라클에서 기본적으로 캐쉬 기능을 사용(기본 캐쉬 사이즈가 20개)
getDeptName(10) => ACCOUNTING 
                오라클 10번인자로 getDeptName실행 했을 때 ACCOUNTING이라는 결과값을 기억(캐싱)
                이후 동일한 인자로 함수를 실행하면 함수를 실행하지 않고 캐싱된 값을 반환

[function 실습 1]
SELECT LPAD(' ', (LEVEL-1)*4, ' ') || deptnm,
        indent(LEVEL)|| deptnm2
FROM dept_h
...
패키지 : 관련되 PL/SQL 블럭을 묶어 놓은 객체  (java의 패키지와 유사) ==>유사한 타입들의 모임
...
CREATE OR REPLACE PACKAGE names AS
    FUNCTION getEmpName (p_empno emp.empno%TYPE) RETURN VARCHAR2;       --선언부
    FUNCTION getDeptName (p_deptno dept.deptno%TYPE) RETURN VARCHAR2;
END names;
/

CREATE OR REPLACE PACKAGE names AS
    FUNCTION getEmpName (p_empno emp.empno%TYPE) RETURN VARCHAR2;       --body(부여부)
    FUNCTION getDeptName (p_deptno dept.deptno%TYPE) RETURN VARCHAR2;
END names;
/
...
Trigger(방아쇠) : 설정한 이벤트에 따라 실행한 로직을 담은 객체
     웹 : 로그인 버튼을 클릭 ==> 사용자 id(아이디) input 값과, password(비번) input 값을 파라미터로 서버로 전송
...
BEGIN
    --테이블 업데이트가 일어 났을때, 기존(:OLD.pass) 비밀번호와 업데이트 하려고 하는 비밀번호(:NEW.pass)가 다를때
...
WHERE userid = 'brown';
--update후 비밀번호 변경 이력이 생김
SELECT *
FROM users_history;
--트리거가 실행되지 않을 조건으로 UPDATE
UPDATE users SET alias = '곰탱이'
WHERE userid = 'brown';
--update후 비밀번호 변경 이력이 생기지 않음
SELECT *
FROM users_history;

트리거 
    장점(시스템을 신규로 만드는 사람  SI)
            : users테이블에 비밀번호를 바꾸는 로직(java코드)은 작성해야한다.
